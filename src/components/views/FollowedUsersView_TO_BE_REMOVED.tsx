...
interface FollowedUsersViewProps {
  posts: Post[];
...
const FollowedUsersView: React.FC<FollowedUsersViewProps> = ({ posts, onServerPostsUpdate }) => {
      const navigate = useNavigate();
      const { publicKey } = useAuth();
      const { fetchAndConvertFollowedUsers, selectedNetwork, apiBaseUrl } = useKaspaPostsApi();
  ...
  const fetchFunctionRef = useRef(fetchAndConvertFollowedUsers);
  ...
  fetchFunctionRef.current = fetchAndConvertFollowedUsers;
...
const loadFollowedUsers = useCallback(async (reset: boolean = true) => {
    try {
      if (reset) {
        setIsLoading(true);
        setError(null);
      ...
      setError(error instanceof Error ? error.message : 'Failed to load followed users');
    } finally {
      if (reset) {
        setIsLoading(false);
      } else {
...
const loadMoreFollowedUsers = useCallback(async () => {
    if (!hasMoreRef.current || isLoadingMoreRef.current) return;
    await loadFollowedUsers(false);
  }, [loadFollowedUsers]);

  // Load followed users on component mount and when network or apiBaseUrl changes
  useEffect(() => {
    if (publicKey) {
      // Use setTimeout to make this non-blocking
      setTimeout(() => loadFollowedUsers(true), 0);
    }
    ...
    const startPolling = () => {
      interval = setInterval(async () => {
        try {
          ...
          const errorMessage = err instanceof Error ? err.message : 'Failed to fetch followed users';
          setError(errorMessage);
          console.error('Error fetching followed users from server:', err);
        }
      }, POLLING_INTERVAL);
    };
      ...
      <div className="sticky top-0 bg-background/80 backdrop-blur-md border-b border-border z-10">
        <div className="p-4">
          <div className="flex items-center space-x-4">
            ...
            <h1 className="text-xl font-bold">Followed users</h1>
          </div>
          {error && (
      ...
      >
        {isLoading && posts.length === 0 ? (
          <div className="p-8 text-center">
            ...
            <p className="text-muted-foreground">Loading followed users...</p>
          </div>
          ...
          <>
            {posts.map((post) => (
              <UserPostCard
                ...
                onUnfollow={(userPubkey) => {
                  // Optimistically remove the user from the followed list
            ...
            {hasMore && isLoadingMore && (
              <div className="p-4 text-center">
                ...
                <p className="text-sm text-muted-foreground mt-2">Loading more followed users...</p>
              </div>
            )}
            ...
            {!hasMore && posts.length > 0 && (
              <div className="p-4 text-center text-muted-foreground text-sm">
                No more followed users to load
              </div>
            )}
          </>
        )}
